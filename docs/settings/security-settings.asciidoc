...
You can enable audit logging to support compliance, accountability and security by capturing who performed an action, what action was performed and when it occurred. For more details see <<xpack-security-audit-logging>>.
...
| `xpack.security.audit.appender` (beta)
  | Optional. Specifies where audit logs should be written to and how they should be formatted. *Default:* console appender
...
[TIP]
============
If you use this setting the legacy audit logger will be disabled and the new audit logger will be used instead.
============
...
----------------------------------------
<1> Filters out successful HTTP request events (Only logs HTTP requests with 4xx and 5xx status code)
<2> Filters out data creation, change and deletion events (Only logs read operations)
...
| `xpack.security.audit.ignore_filters[].categories[]` (beta)
  | List of values matched against the `event.category` field of an audit event. See [ECS categorization field](https://www.elastic.co/guide/en/ecs/current/ecs-allowed-values-event-category.html) for allowed values.

| `xpack.security.audit.ignore_filters[].types[]` (beta)
  | List of values matched against the `event.type` field of an audit event. See [ECS type field](https://www.elastic.co/guide/en/ecs/current/ecs-allowed-values-event-type.html) for allowed values.

| `xpack.security.audit.ignore_filters[].outcomes[]` (beta)
  | List of values matched against the `event.outcome` field of an audit event. See [ECS outcome field](https://www.elastic.co/guide/en/ecs/current/ecs-allowed-values-event-outcome.html) for allowed values.
|===
